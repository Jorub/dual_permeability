module schwarz_dd_time_test
	...
	logical :: dt_fine, first_run, picard_done
	logical, save :: reset_domains = .true.
	integer :: ierr_loc
...
! 	
	...
	allocate(tempvect(2, ubound(pde_common%xvect,1), 4))
	  ...
	  iters(i) = ddcoarse_mesh(i)%iter_count
	end do
        
        print *, "max iter",  maxval(ddcoarse_mesh(:)%iter_count), itcount, maxval(iters)
...
!         print *, reset_domains, itcount, ierr ; stop
	...
	iter_criterion = 5.5
	  ...
	  time_step = max(0.5*time_step, 0.95*dtmin)
...
! 	  end ifmake
	  ...
	  if (vyreseno() == ubound(subdomain,1)) then
	  	  write(unit=file_dt, fmt=*) subdomain(:)%tmpval
	...
	do i=1, coarse_elements%kolik
	  ddcoarse_mesh(i)%iter_count = iters(i)
	end do
        
        print *, "max iter",  maxval(ddcoarse_mesh(:)%iter_count), itcount, maxval(iters), iter_criterion
...
! 	  print *, "dfs"
	    ddcoarse_mesh(j)%iter_count = itcount + 1
	  end if
	end do
